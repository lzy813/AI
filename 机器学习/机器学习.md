...
- 什么是交叉验证？
  - 是一种<font  color="red">**数据集的分割方法**</font>，将训练集划分为 <font  color="red">**n 份**</font>，拿<font  color="red">**一份做验证集**</font>、<font  color="red">**其他 n-1 份做训练集**</font>
...
- 手写数字识别.csv 包含从0到9的手绘数字的灰度图像
- 每个图像高 28像素，宽28像素，共784个像素
...
介绍：
    每张图片都是28*28像素组成，即：csv文件文件中每一行都有784个像素点，表示图片（每个像素的）的颜色
    最终构成图片
"""
...
def show_digit(idx):
    ...
    # 4、查看用户传入的索引对应的图片
    print(f'用户传入的索引对应的数字是：{y.iloc[idx]}')
    print(f'查看所有标签的分布情况：{Counter(y)}')

    # 5、查看下x的形状
    print(x.iloc[idx].shape)        # (784,)，将784转成 (28, 28)
...
def train_model():
    ...
    print(f'y的分布情况：{Counter(y)}')
    # 2.4 对特征列（拆分前）进行归一化
    x = x / 255
    # 2.5 拆分训练集和测试集
    # 入参：参数1：特征列  参数2：标签列  参数3：测试集的拆分比例  参数4：随机种子  参数5：参考y值进行抽取，保持标签的比例（数据均衡）
...
def test_model():
    ...
    # 2、加载模型
    model = joblib.load('./model/手写数字识别.pkl')

    # 3、模型预测
    # 3.1 查看数据集转化
    print(img.shape)            # (28, 28)
    print(img.reshape(1, 784).shape)  # (1, 784)
    print(img.reshape(1, -1).shape)   # (1, 784)

    # 3.2 具体的转化动作，要归一化（因为要和训练集保持同步），这里的imread读出来已经归一化了，所以不需要了
    img = img.reshape(1, -1)

    # 3.3 模型预测
    y_read = model.predict(img)
    print(f'你绘制的图片是：{y_read}')
...
- 举个例子：假如有了身高体重数据，来了一个人的身高信息，需要预测其体重
  - 思路：先从已知身高X和体重Y中找规律，再预测
  - 数学问题：<font color="red">**用一条线来拟合身高和体重之间的关系，再对新数据进行预测**</font>

![身高体重例子](图片/线性回归/身高体重例子.PNG)

- 线性回归概念

![线性回归概念](图片/线性回归/线性回归概念.PNG)

- 线性回归分类
  - 一元线性回归
    - y  = wx + b
    - <font color="red">**目标值只与一个因变量有关系**</font>
  - 多元线性回归
    - y = w1x1 + w2x2 + ... + wnxn + b
    - <font color="red">**目标值只与多个因变量有关系**</font>
...
- 代码API：estimator = LinearRegression()
...
线性回归介绍（Linear Regressor）
    概述/目的：
        用线性公式 来描述 多个自变量（特征）和 1个因变量（标签）之间的关系的，对其关系进行建模，基于特征预测标签
        线性回归问题属于：监督学习，即：有特征有标签，且标签是连续的
    分类：
        一元线性回归：1个特征列 + 一个标签列
        多元线性回归：多个特征列 + 一个标签列
    公式：
        一元线性回归：
            y = kx + b ==> wx + b
                k：数学中叫斜率，机器学习中叫 weight(权重)，简称w
                b：数学中叫截距，机器学习中叫 bias(偏置)，简称b
        多元线性回归：
            y = w1x1 + w2x2 + ... + wnxn + b
              = w的转置 * x + b
"""
...
# 4.2 具体的训练动作
estimator.fit(x_train, y_train)
# 4.3 因为是线性回归问题，可以查看：斜率（w, 权重），截距（b, 偏置）
print(f'权重：{estimator.coef_}')
print(f'偏置：{estimator.intercept_}')

# 5、模型预测
y_pre = estimator.predict(x_test)
print(f'预测的体重为：{y_pre}')            # 70.3047619
...
- 误差概念：<font color="red">**预测值y - 真实值y** </font> 就是误差
- 损失函数：<font color="red">**衡量每个样本预测值与真实值关系的函数** </font>，也叫代价函数、成本函数、目标函数
- 红色直线能更好的拟合所有点，也就是误差最小；<font color="red">**误差和最小** </font>

![损失函数](图片/线性回归/损失函数.PNG)
...
![损失函数初始](图片/线性回归/损失函数初始.PNG)

- <font color="red">**当损失函数取最小值时，得到的k值就是最小值** </font>

![求损失函数最小值](图片/线性回归/求损失函数最小值.PNG)



### 2.4 损失函数的种类和数学表达

- <font color="red">**最小二乘** </font>：每个样本点误差的平方和
- <font color="red">**MSE（Mean Square Error，均方误差）** </font>：每个样本点误差的平方和 / 样本个数
- <font color="red">**RMSE（Root Mean Square Error，均方根误差）** </font>：均方误差 开平方根
- <font color="red">**MAE（Mean Absolute Error，均绝对误差）** </font>：每个样本点误差的绝对值和 / 样本个数

![损失函数种类和数学表达](图片/线性回归/损失函数种类和数学表达.PNG)



### 2.5 损失函数求解步骤

![线性回归问题求解步骤](图片/线性回归/线性回归问题求解步骤.PNG)
...
- 常见数学名称
  ...
  - <font color="red">**标量（Scalar）：只有大小，没有方向，就是一个单独的数字**</font>

    - 例子：温度 25℃、身高 180cm、价格 99 元、年龄 20
    - 数学表示：一个数，比如 a=5

  - <font color="red">**向量（Vector）：一组有序的数字，有大小 + 方向**</font>

    - 可以理解成：**一行或一列数字**。
    - 例子：速度 (3, 4)、坐标 (x, y, z)、特征 [身高，体重，年龄]
    - 数学表示：v=123或[1,2,3]

  - <font color="red">**矩阵（Matrix）：二维表格，由多行多列向量组成**</font>

    - 例子：图片灰度像素表、数据表、变换矩阵
    - 数学表示：M=[123，456]，这是一个 2 行 3 列的矩阵

  - <font color="red">**张量（Tensor）：多维数组的统称，是上面所有概念的父级**</font>
...
![导数概念](图片/线性回归/导数概念.png)

- 导数的几何意义

![导数的几何意义](图片/线性回归/导数的几何意义.png)

- 常见导数公式

![常见导数公式](图片/线性回归/常见导数公式.png)

- 导数四则运算

![导数四则运算](图片/线性回归/导数四则运算.png)
...
![矩阵转置](图片/线性回归/矩阵转置.png)

- 矩阵乘法性质

![矩阵乘法性质](图片/线性回归/矩阵乘法性质.png)

- 矩阵转置性质

![矩阵转置性质](图片/线性回归/矩阵转置性质.png)

## 4、正规方程法

### 4.1 一元线性回归解析

![一元线性回归解析1](图片/线性回归/一元线性回归解析1.png)

![一元线性回归解析2](图片/线性回归/一元线性回归解析2.png)



![一元线性回归解析3](图片/线性回归/一元线性回归解析3.png)

![一元线性回归解析4](图片/线性回归/一元线性回归解析4.png)
...
![多元线性回归解析1](图片/线性回归/多元线性回归解析1.png)



- 多元线性回归解析2![多元线性回归解析2](图片/线性回归/多元线性回归解析2.png)

- 多元线性回归解析例子

![多元线性回归解析例子](图片/线性回归/多元线性回归解析例子.png)

- 总结： <font color="red">**正规方程法适用于特征数量较少的情况。当特征数量较大时，计算逆矩阵的复杂度会显著增加，此时梯度下降法更为适用。**</font>
...
- <font color="red">**逻辑回归通过sigmoid激活函数将线性回归的输出映射到[0, 1]区间，然后结合阈值，划分正负样本**</font>
  - 属于<font color="red">**监督学习**</font>：<font color="blue">**有特征有标签**</font>
  - <font color="red">**标签是离散（分类）的**</font>
  - 适用于<font color="red">**二分类**</font>
...
![sigmoid函数](图片/逻辑回归/sigmoid函数.PNG)

- <font color="red">**作用：把(-∞, +∞) 映射到 (0, 1)**</font>
- 数学性质：
  - 单调递增函数
  - 拐点在x = 0, y = 0.5的位置
  - x=0轴右边为正样本，左边为负样本

![sigmoid函数图](图片/逻辑回归/sigmoid函数图.PNG)

![sigmoid函数解析](图片/逻辑回归/sigmoid函数解析.PNG)

#### 1.3.2 概率论

- 概率：事件发生的可能性
  - <font color="red">**联合概率**</font>：指两个或多个随机变量<font color="red">**同时发生的概率**</font>
    - 记法：写作 P(A, B)、P(A ∩ B) 或 P(AB)
    - 直观理解：从整个样本空间来看，\(A\) 和 \(B\) 的重叠区域所占的比例
    - 独立事件特例：若 \(A\) 与 \(B\) 相互独立，则 P(AB) = P(A) * P(B)
  - <font color="red">**条件概率**</font>：表示事件A<font color="red">**在另一个事件B已经发生的条件下的发生概率**</font>
    - 记法：写作 P(A|B)
    - P(A|B) = P(AB)/{P(B) （其中 \(P(B) > 0\)）
  - 比如：
    - 北京早上堵车的可能性PA=0.7，中午堵车的可能性PB=0.3，晚上堵车的可能性PC=0.4
    - 联合概率：周一周二早上同时堵车的概率为：PA * PA = 0.7 * 0.7 = 0.49
    - 条件概率：周一早上堵车的情况下，中午再堵车的概率为：P(A|B) = P(AB)/P(B) = (P(A) * P(B)) / P(B) = (0.7 *0.3) / 0.3 = 0.7



#### 1.3.3 极大似然估计

- 核心思想：<font color="red">**根据观测到的结果来估计模型算法中的未知参数**</font>

![极大似然估计](图片/逻辑回归/极大似然估计.PNG)
...
- 逻辑回归概念（Logistic Regression）
  ...
  - 输出是（0,1）之间的值
- 基本思想
  - 利用线性模型 f(x) = WᵀX + b <font color="red">**根据特征的重要性计算出一个值**</font>
  - <font color="red">**再使用sigmoid函数将f(x)的输出值映射为概率值**</font>
